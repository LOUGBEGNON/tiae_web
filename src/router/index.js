...
import LayoutDashboard from "@/layouts/variations/LayoutDasboard.vue";
...
const HomeDashboard = () => import("@/views/dashboard/HomeDashboardView.vue");
...
router.beforeEach(async (to, from, next) => {
  ...
  console.log(publicOnlyRoutes.includes(to.name))
  console.log(to.name)

  // Redirection si utilisateur NON connecté essaie d’aller vers une page protégée
  if (to.meta.requiresAuth && !auth.user) {
    return next({ name: "login" })
  }
  // Redirection si utilisateur DEJA connecté essaie d’aller vers une page publique uniquement
  if (to.meta.publicOnly && auth.user) {
    return next({ name: "dashboard" }) // ou autre page privée
  }



  // Si l'utilisateur est connecté et essaie d'accéder à une page d'authentification
  if (auth.token && publicOnlyRoutes.includes(to.name)) {
    next({ name: 'login' }) // Redirige vers la page d'accueil ou dashboard
  } else {
